...
export class Renderer {
    ...
    renderGrid(context) {
        ...
        for (let y = -camHeight; y < camHeight; y++) {
            const pos = new Vector(camPosition.x - camWidth, camPosition.y + y).scale(TILE_SIZE);
            context.beginPath();
    ...
    wordToScreenPosition(pos) {
        return new Vector(
            pos.x - this.world.cameraPosition.x + this.gameWidth / 2,
            pos.y - this.world.cameraPosition.y + this.gameHeight / 2
        ).round();
    }
